...
public class SuspensionGraph : GraphBase
{
    private void Start()
    {
    ...
    IEnumerator Co_UpdateRendering()
    {
        while (true)
        {
            ...
            tempWheelData[0] = (ws.wheelCol.suspensionDistance - ws.contactDepth) * 1000.0f;

            ws = vehicle.wheelState[1];
            tempWheelData[1] = (ws.wheelCol.suspensionDistance - ws.contactDepth) * 1000.0f;

            ws = vehicle.wheelState[2];
            tempWheelData[2] = (ws.wheelCol.suspensionDistance - ws.contactDepth) * 1000.0f;

            ws = vehicle.wheelState[3];
            tempWheelData[3] = (ws.wheelCol.suspensionDistance - ws.contactDepth) * 1000.0f;

            // 그래프 보간
            float speedChart = 4f + tempVehicleData[VehicleData.Speed] / 20.0f;
            float wheelFLChart = 1f + tempWheelData[0] / 20.0f;
            float wheelFRChart = 1f + tempWheelData[1] / 20.0f;
            float wheelRLChart = 1f + tempWheelData[2] / 20.0f;
            float wheelRRChart = 1f + tempWheelData[3] / 20.0f;
